...
public class MjpegInputStream extends DataInputStream {
    ...
    public MjpegInputStream(Context context, InputStream in) {
    ...
    public int readMjpegFrame(Bitmap bmp) throws IOException {
        ...
        //Log.d(TAG, "1frameData : " + new String(frameData));
        readFully(frameData, 0, mContentLength);

        if(count == 0)
           bytesToHex(frameData);

        if (count++ % skip == 0) {
            //Log.d(TAG,  " lenght " + bytesToHex(frameData).length() +  " , 2frameData : " + bytesToHex(frameData));
            return ((MainActivity)MyApp.content).pixeltobmp(frameData, mContentLength, bmp);
        } else {
            return 0;
        }
    }
