...
public abstract class VideoStream extends MediaStream {
	...
	protected MySurfaceView mMySurfaceView = null;
	...
	private int ACTUAL_CAMERA = 1;

	public void switchCamera() throws RuntimeException, IOException {
		ACTUAL_CAMERA = ACTUAL_CAMERA + 1;
		if(ACTUAL_CAMERA > MAX_CAMERAS)
			ACTUAL_CAMERA = 1;

		Log.d(TAG, "switching camera to : " + ACTUAL_CAMERA);

			boolean streaming = mStreaming;
			boolean previewing = mCamera != null && mCameraOpenedManually;

			if(ACTUAL_CAMERA == 1) {    //built android camera
				if (Camera.getNumberOfCameras() == 1)
					throw new IllegalStateException("Phone only has one camera !");

				mCameraId = (mCameraId == CameraInfo.CAMERA_FACING_BACK) ? CameraInfo.CAMERA_FACING_FRONT : CameraInfo.CAMERA_FACING_BACK;	//CameraInfo.CAMERA_FACING_BACK;
			...
			}else if(ACTUAL_CAMERA == 2){	//external ip camera, must start also with wifi connection
				//detachCameraFromTexture();
				stopPreview();
				...
				//Log.d(TAG, "executing DoRead..");

				//String url = "http://192.168.2.1:80/?action=stream";
	 ...
	 */
	public synchronized void setSurfaceView(MySurfaceView view) {
		mMySurfaceView = view;
		if (mSurfaceHolderCallback != null && mMySurfaceView != null && mMySurfaceView.getHolder() != null) {
			mMySurfaceView.getHolder().removeCallback(mSurfaceHolderCallback);
		}
		if (mMySurfaceView.getHolder() != null) {
			...
			mMySurfaceView.getHolder().addCallback(mSurfaceHolderCallback);
			mSurfaceReady = true;
		}
	}
	...
	public synchronized void stop() {
		if (mCamera != null) {
			...
			if (mMode == MODE_MEDIACODEC_API_2) {
				((MySurfaceView) mMySurfaceView).removeMediaCodecSurface();
			}
			super.stop();
	...
	/*
	public synchronized void stopOnlyCameraPreview(){
		mMySurfaceView.internalCameraRelease();
	}
	*/
	...
	protected void encodeWithMediaRecorder() throws IOException, ConfNotSupportedException {
		...
		try {
			...
			mMediaRecorder.setPreviewDisplay(mMySurfaceView.getHolder().getSurface());
	...
	public void detachCameraFromTexture(){
		if(mCamera != null){
			try {
	...
	protected void encodeWithMediaCodecMethod2() throws RuntimeException, IOException {
		...
		((MySurfaceView) mMySurfaceView).addMediaCodecSurface(surface);
		mMediaCodec.start();
	...
	protected synchronized void createCamera() throws RuntimeException {
		...
		if (mMySurfaceView.getHolder() == null || !mSurfaceReady)
			throw new InvalidSurfaceException("Invalid surface !");
		...
		if (mCamera == null) {
			...
			try {
				...
				try {
					if (mMode == MODE_MEDIACODEC_API_2) {
						mMySurfaceView.startGLThread();
						mCamera.setPreviewTexture(mMySurfaceView.getSurfaceTexture());
					} else {
						mCamera.setPreviewDisplay(mMySurfaceView.getHolder());
					}
				} catch (IOException e) {
					throw new InvalidSurfaceException("Invalid surface !");
				}
	...
	protected synchronized void updateCamera() throws RuntimeException {
		...
		mMySurfaceView.requestAspectRatio(ratio);
	...
	/* MJPEG INPUT STREAM FROM EXTERNAL IP CAMERA
	...
	private class DoRead extends AsyncTask<String, Void, MjpegInputStream> {
		protected MjpegInputStream doInBackground(String... url) {
			...
			if (DEBUG) Log.d(TAG, "1. Sending http request " + URI.create(url[0]).toString());
			try {
				res = httpclient.execute(new HttpGet(URI.create(url[0])));
				if (DEBUG) {
					Log.d(TAG, "2. Request finished, status = " + res.getStatusLine().getStatusCode()); // + " response : " + EntityUtils.toString(res.getEntity()));
					//if(res.getEntity() != null)
					//   Log.d(TAG, "3. response : " + res.getEntity().getContent());
				}
				if (res.getStatusLine().getStatusCode() == 401) {
					//You must turn off camera User Access Control before this will work
					return null;
				}
				return new MjpegInputStream(MyApp.getContext(), res.getEntity().getContent());
		...
		protected void onPostExecute(MjpegInputStream result) {
			//first stop built camera preview and disable flash
			stopPreview();	//possible that also stop live transcoding from surface view into mediacodec packets
			mFlashEnabled = false;
			mSurfaceView.setExternalCam(true);
			mSurfaceView.initMjpegView(MyApp.getContext());
			mSurfaceView.setSource(result);
			if (result != null) {
				result.setSkip(1);
			...
			mSurfaceView.setDisplayMode(mSurfaceView.SIZE_BEST_FIT);



			//if was streaming before, start again !
			try {
				if (mStreaming) start();	//thats why we must start straming again if was running before !
			} catch (IOException e) {
				e.printStackTrace();
			}
		}
